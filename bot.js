...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("PrzyjacieleRP Serwer Status connect 88.214.58.137:30478", "https://cdn.discordapp.com/attachments/769511800466309141/810864905451733002/logo.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ PrzyjacieleRP' : '⚫ PrzyjacieleRP')
    .setTimestamp(new Date())
    .addField('Jak dołączyć do serwera','Odpalamy FiveM klikamy f8, wpisujemy connect 88.214.58.137:30478',false)
    if (STATUS !== undefined)
    {
      embed.addField(':warning: Aktualny status serwera:',`${STATUS}\n\u200b\n`);
  ...
  const offline = function() {
    ...
    .addField('kolejka','?',true)
    .addField('Gracze Online','?\n\u200b\n',true);
    sendOrUpdate(embed);
    LAST_COUNT = null;
  };
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        let embed = UpdateEmbed()
        .addField('Status Serwer',':white_check_mark: Online',true)
        .addField('Kolejka',queue === 'Enabled' || queue === undefined ? '0' : queue.split(':')[1].trim(),true)
        .addField('Gracze Online',`${players.length}/${MAX_PLAYERS}\n\u200b\n`,true);
        // .addField('\u200b','\u200b\n\u200b\n',true);
        if (players.length > 0) {
          // method D
          const fieldCount = 3;
  ...
  bot.on('ready',() => {
    ...
    bot.user.setActivity('Przyjaciele RP',{'url':'https://www.twitch.tv/carbonnaura','type':'STREAMING'});
